...
class KimiK3ReasoningParser(ReasoningParser):
    ...
    def count_reasoning_tokens(self, token_ids: Sequence[int]) -> int:
        """Count generated token IDs inside K3's multi-token think channel."""
        if not self._thinking_enabled:
            return 0
        close_idx = _subseq_index(token_ids, self._think_close_ids)
        if close_idx == -1:
            open_idx = _subseq_index(token_ids, self._think_open_ids)
            if open_idx == -1:
                # The generation prefix commonly consumes the open marker.
                # Until a close marker arrives, all generated IDs are thinking.
                return len(token_ids)
            return len(token_ids) - open_idx - len(self._think_open_ids)
        prefix = token_ids[:close_idx]
        open_idx = _subseq_index(prefix, self._think_open_ids)
        start = open_idx + len(self._think_open_ids) if open_idx != -1 else 0
        return max(close_idx - start, 0)
