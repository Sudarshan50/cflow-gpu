...
def _count_reasoning_tokens(
    parser: Parser | None, token_ids: GenericSequence[int]
) -> int:
...
def _completion_tokens_details(reasoning_tokens: int) -> CompletionTokenUsageInfo:
...
class OpenAIServingChat(GenerateBaseServing):
        ...
        try:
            async for res in result_generator:
                ...
                for output in res.outputs:
                    ...
                    output_token_ids[i].extend(output.token_ids)
                    ...
                    if include_continuous_usage:
                        ...
                        chunk.usage = UsageInfo(
                            ...
                            completion_tokens_details=_completion_tokens_details(
                                _count_reasoning_tokens(parser, output_token_ids[i])
                            ),
                        )
            ...
            if include_usage:
                ...
                final_usage = UsageInfo(
                    ...
                    completion_tokens_details=_completion_tokens_details(
                        sum(
                            _count_reasoning_tokens(parsers[i], output_token_ids[i])
                            for i in range(num_choices)
                        )
                    ),
                )
    ...
    async def chat_completion_full_generator(
        ...
        num_reasoning_tokens = sum(
            _count_reasoning_tokens(parser, output.token_ids)
            for output in final_res.outputs
        )
        usage = UsageInfo(
            prompt_tokens=num_prompt_tokens,
